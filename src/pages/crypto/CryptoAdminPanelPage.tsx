...
export default function CryptoAdminPanelPage({
  ...
  const disputes = (orders as any[]).filter((o) => o.status === "disputed");
  const buyers   = (accounts as any[]).filter((a) => !a.isAdmin && !a.isSeller);
  const sellers  = (accounts as any[]).filter((a) => a.isSeller && !a.isAdmin);

  const safeNotify = (msg: string) => { try { notify?.(msg); } catch (_) {} };
  ...
  const fmtUSD = (fcfa: number) => `$${((fcfa || 0) / 615).toFixed(2)}`;
  ...
  const PromoteModal = () => {
    ...
    const activate = () => {
      updateAcc(acc.id, { isSeller: true, sellerStatus: "active", sellerLimits: { ...form, expiresAt: addDays(form.activeDays) }, sellerActivatedAt: new Date().toISOString() });
      promoteToSeller?.(acc.id);
      setPromoteModal(null);
      safeNotify(`✅ ${acc.name} activé comme annonceur pour ${form.activeDays} jours.`);
    };
      ...
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16, overflowY:"auto" }}>
        <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:20, padding:28, width:"100%", maxWidth:540, maxHeight:"92vh", overflowY:"auto" }}>
          ...
          <div style={{ background:"rgba(239,68,68,0.07)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:12, padding:16, marginBottom:16 }}>
            <div style={{ fontWeight:800, fontSize:14, color:C.red, marginBottom:4 }}>🔒 Dépôt de Garantie (FCFA)</div>
            <div style={{ color:C.muted, fontSize:12, marginBottom:12 }}>Montant déposé par le vendeur. En cas de litige non résolu, ce fonds sera transféré à l'acheteur lésé.</div>
          ...
          <div style={{ marginBottom:22 }}>
            ...
            {form.allowedCryptos.length === 0 && <div style={{ color:C.red, fontSize:12, marginTop:8 }}>⚠ Sélectionnez au moins une crypto</div>}
          </div>
  ...
  const SellerModal = () => {
    ...
    const expiryBg    = isExpired ? "rgba(239,68,68,0.1)"  : days!==null&&days<=7 ? "rgba(245,158,11,0.1)"  : "rgba(16,185,129,0.07)";
    const expiryBd    = isExpired ? "rgba(239,68,68,0.3)"  : days!==null&&days<=7 ? "rgba(245,158,11,0.3)"  : "rgba(16,185,129,0.2)";
    const expiryColor = isExpired ? C.red                  : days!==null&&days<=7 ? C.gold                  : C.green;

    const save = () => { updateAcc(acc.id, {sellerStatus, sellerLimits:form}); setSellerModal(null); safeNotify(`✅ ${acc.name} mis à jour.`); };
    const extendAccount = () => {
      const updated = {...form, expiresAt:addDays(extendDays)};
      setForm(updated);
      updateAcc(acc.id, {sellerStatus:"active", sellerLimits:updated});
      safeNotify(`✅ Compte de ${acc.name} prolongé de ${extendDays} jours.`);
    };
    ...
    return (
      <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.82)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16, overflowY:"auto" }}>
        <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:20, padding:28, width:"100%", maxWidth:560, maxHeight:"92vh", overflowY:"auto" }}>
          ...
          {acc.password && (
            <div style={{ background:"rgba(245,158,11,0.07)", border:"1px solid rgba(245,158,11,0.25)", borderRadius:12, padding:"12px 16px", marginBottom:16 }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                ...
                <button onClick={() => setShowPwd(p=>!p)} style={{ background:"rgba(255,255,255,0.06)", border:`1px solid ${C.border}`, borderRadius:8, padding:"7px 13px", color:C.muted, cursor:"pointer", fontSize:12, fontWeight:700 }}>
                  {showPwd?"🙈 Masquer":"👁 Voir"}
                </button>
              </div>
            </div>
          ...
          <div style={{ background:expiryBg, border:`1px solid ${expiryBd}`, borderRadius:12, padding:14, marginBottom:16 }}>
            <div style={{ fontSize:12, fontWeight:700, color:expiryColor, marginBottom:6 }}>{isExpired?"⛔ COMPTE EXPIRÉ":days!==null&&days<=7?"⚠️ EXPIRE BIENTÔT":"✅ COMPTE ACTIF"}</div>
          ...
          <div style={{ background:"rgba(239,68,68,0.07)", border:"1px solid rgba(239,68,68,0.22)", borderRadius:12, padding:14, marginBottom:16 }}>
            <div style={{ fontWeight:800, fontSize:14, color:C.red, marginBottom:6 }}>🔒 Dépôt de Garantie</div>
            <div style={{ color:C.muted, fontSize:12, marginBottom:10 }}>En cas de litige non résolu, ce montant sera transféré à l'acheteur lésé.</div>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              {(["active","restricted","blocked"] as const).map(s => {
                const col = s==="active"?C.green:s==="restricted"?C.gold:C.red;
                return (
                  <button key={s} onClick={() => setSellerStatus(s)}
                    style={{ padding:"8px 16px", borderRadius:9, fontSize:13, fontWeight:700, cursor:"pointer", border:`1px solid ${sellerStatus===s?col:C.border}`, background:sellerStatus===s?`${col}26`:"transparent", color:sellerStatus===s?col:C.muted }}>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
            ...
            <button onClick={() => { if(window.confirm(`Supprimer le compte vendeur de ${acc.name} ?`)){updateAcc(acc.id,{isSeller:false,sellerStatus:null,sellerLimits:null});setSellerModal(null);safeNotify(`${acc.name} retiré des vendeurs.`);}}}
              style={{ padding:"13px 18px", borderRadius:12, border:"1px solid rgba(239,68,68,0.3)", cursor:"pointer", background:"rgba(239,68,68,0.12)", color:C.red, fontWeight:700, fontSize:13 }}>🗑 Révoquer</button>
  ...
  const DisputeModal = () => {
    ...
    const seller = (accounts as any[]).find((a: any) => a.id===order.sellerId);
    ...
    const resolve = () => {
      ...
      if (favor==="buyer") {
        ...
        safeNotify(`✅ Litige résolu — ${fmt(transferAmount)} FCFA transféré à l'acheteur.`);
      } else {
        setOrders?.((prev: any[]) => prev.map((o: any) => o.id===order.id?{...o,status:"confirmed",disputeNote:note}:o));
        safeNotify(`✅ Litige résolu en faveur du vendeur.`);
      }
        ...
        <div style={{ background:C.bgCard, border:"1px solid rgba(239,68,68,0.4)", borderRadius:20, padding:28, width:"100%", maxWidth:520, maxHeight:"92vh", overflowY:"auto" }}>
          ...
          <div style={{ marginBottom:18 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
              {(["buyer","seller"] as const).map(f => (
                <button key={f} onClick={() => setFavor(f)}
                  style={{ padding:14, borderRadius:12, textAlign:"left", cursor:"pointer", fontSize:14, fontWeight:700, border:`2px solid ${favor===f?(f==="buyer"?C.blue:C.purple):C.border}`, background:favor===f?(f==="buyer"?"rgba(59,130,246,0.12)":"rgba(139,92,246,0.12)"):"transparent", color:favor===f?(f==="buyer"?C.blue:C.purple):C.muted }}>
                  <div>{f==="buyer"?"👤 Acheteur — Rembourser + Transférer fonds":"🏪 Vendeur — Transaction confirmée"}</div>
                  <div style={{ fontSize:12, fontWeight:500, marginTop:4, color:C.muted }}>{f==="buyer"?"Le dépôt de garantie du vendeur sera déduit et remis à l'acheteur.":"La crypto a bien été envoyée. Litige rejeté."}</div>
                </button>
              ))}
            </div>
          </div>
          {favor==="buyer"&&(
            <div style={{ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:12, padding:14, marginBottom:16 }}>
              <label style={{...st.label, color:C.red}}>💸 Montant à transférer depuis le dépôt (FCFA)</label>
              ...
              {transferAmount>sellerReserve&&<div style={{ color:C.red, fontSize:12, marginTop:6 }}>⚠ Montant supérieur au dépôt ({fmt(sellerReserve)} FCFA)</div>}
              <div style={{ color:C.muted, fontSize:12, marginTop:6 }}>Dépôt restant : <strong style={{ color:Math.max(0,sellerReserve-transferAmount)===0?C.red:C.green }}>{fmt(Math.max(0,sellerReserve-transferAmount))} FCFA</strong></div>
            </div>
          )}
          <div style={{ marginBottom:20 }}>
            <label style={st.label}>📝 Note de résolution (optionnel)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Ex: Vendeur n'a pas fourni de preuve d'envoi..." rows={3} style={{...st.input, resize:"vertical", fontFamily:"inherit"}} />
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={resolve} disabled={!favor}
              style={{ flex:1, padding:"13px", borderRadius:12, border:"none", fontWeight:800, fontSize:14, cursor:favor?"pointer":"not-allowed", background:!favor?"rgba(255,255,255,0.06)":favor==="buyer"?"linear-gradient(135deg,#3b82f6,#1d4ed8)":"linear-gradient(135deg,#8b5cf6,#6d28d9)", color:!favor?C.muted:"#fff" }}>
    ...
    <>
      ...
      <SidebarLayout user={user} onLogout={onLogout} navigate={navigate} active={active} setActive={setActive} menu={menu} title={titles[active]||"Administration"}>
        ...
        {active==="overview"&&(
          <>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:24 }}>
              {[
                { l:"Utilisateurs",    v:(accounts as any[]).filter((a:any)=>!a.isAdmin).length,                                                i:"👥", col:C.purple },
                { l:"Vendeurs actifs", v:sellers.filter((a:any)=>a.sellerStatus==="active").length,                                            i:"🏪", col:C.green  },
                { l:"Commandes",       v:(orders as any[]).length,                                                                              i:"📦", col:C.blue   },
                { l:"Annonces",        v:(offers as any[]).length,                                                                              i:"🏷️", col:C.gold   },
              ...
              <Card>
                ...
                {(orders as any[]).length===0
                  ...
                  : Object.entries(STATUS).map(([k,v])=>{
                      const count=(orders as any[]).filter((o:any)=>o.status===k).length;
                      return (
              ...
              : buyers.map((acc:any)=>(
                  <div key={acc.id} style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:16, padding:18, marginBottom:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                      ...
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                        {acc.password&&<button onClick={()=>setPasswordModal(acc)} style={{...st.btn("secondary","sm")}}>🔑 Mot de passe</button>}
        ...
        {active==="sellers"&&(
          <div>
            {sellers.length===0
              ? <div style={{ textAlign:"center", padding:"40px 0", color:C.muted }}><div style={{ fontSize:40, marginBottom:10 }}>🏪</div><p>Aucun vendeur actif</p></div>
              : sellers.map((acc:any)=>{
                  const sStatus=SELLER_STATUS[acc.sellerStatus]||SELLER_STATUS.active;
                  const days=daysLeft(acc.sellerLimits?.expiresAt??null);
                  const isExpired=days!==null&&days<=0;
                  return (
                      ...
                      <div style={{ display:"flex", alignItems:"flex-start", gap:14, flexWrap:"wrap" }}>
                        ...
                        <div style={{ flex:1, minWidth:0 }}>
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            ...
                            {isExpired
                              ? <span style={{ background:"rgba(239,68,68,0.15)", color:C.red, padding:"3px 10px", borderRadius:100, fontSize:11, fontWeight:700 }}>⛔ Expiré</span>
                              : days!==null
                                ? <span style={{ background:days<=7?"rgba(245,158,11,0.15)":"rgba(16,185,129,0.1)", color:days<=7?C.gold:C.green, padding:"3px 10px", borderRadius:100, fontSize:11, fontWeight:700 }}>⏱ {days}j restant{days>1?"s":""}</span>
                                : null}
                          ...
                          {(acc.sellerLimits?.allowedCryptos?.length??0)>0&&(
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                          ...
                          {acc.sellerStatus!=="blocked"
                            ? <button onClick={()=>{updateAcc(acc.id,{sellerStatus:"blocked"});safeNotify(`${acc.name} bloqué.`);}} style={{...st.btn("red","sm")}}>🚫 Bloquer</button>
                            : <button onClick={()=>{updateAcc(acc.id,{sellerStatus:"active"});safeNotify(`${acc.name} débloqué. ✅`);}} style={{...st.btn("green","sm")}}>✅ Débloquer</button>}
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div>
            {disputes.length===0
              ...
              : disputes.map((order:any)=>{
                  const cr=getCr(order.crypto); const seller=(accounts as any[]).find((a:any)=>a.id===order.sellerId); const reserve:number=seller?.sellerLimits?.reserve??0;
                  return (
                    <div key={order.id} style={{ background:C.bgCard, border:"1px solid rgba(239,68,68,0.35)", borderRadius:16, padding:18, marginBottom:14 }}>
                      <div style={{ display:"flex", alignItems:"flex-start", gap:12, flexWrap:"wrap", marginBottom:14 }}>
                        ...
                        <div style={{ flex:"0 0 auto", textAlign:"right" }}><div style={{ fontSize:11, color:C.muted, marginBottom:2 }}>Dépôt vendeur</div><div style={{ fontSize:16, fontWeight:900, color:C.gold }}>{fmt(reserve)} FCFA</div></div>
                      </div>
                      <button onClick={()=>setDisputeModal(order)} style={{ width:"100%", padding:"11px", borderRadius:11, border:"none", cursor:"pointer", background:"linear-gradient(135deg,#ef4444,#dc2626)", color:"#fff", fontWeight:800, fontSize:14 }}>⚖️ Résoudre le litige & Transférer fonds</button>
                    </div>
                  );
                })}
          </div>
                  ...
                  return (
                    <div key={offer.id} style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:16, padding:18 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                        ...
                        <button onClick={()=>{setOffers?.((offers as any[]).filter((o:any)=>o.id!==offer.id));safeNotify("Annonce supprimée.");}} style={{...st.btn("danger","sm")}}>🗑 Supprimer</button>
                      </div>
                    </div>
                  );
                })}
