...
import { C, getCr, fmt, makeStyles } from "./crypto-constants";
...
export default function CryptoBuyPage({ navigate, user, offer, orders, setOrders, accounts, notify }: any) {
  ...
  const vw = (addr: string) => { if(!addr){setWErr("");return;} setWErr(cr.addrRegex.test(addr)?"":` Format invalide. Ex : ${cr.addrEx}`); };
  ...
  const handleConfirm = () => {
    ...
    setTimeout(()=>{
      ...
      setOrderId(id);
      setOrders((p: any)=>[{ id, crypto:offer.crypto, amount:parseFloat(amount), amountFCFA:baseAmount, networkFee, totalFCFA, walletAddr:wallet, seller:offer.sellerName, sellerId:offer.sellerId, buyerId:user.id, buyerName:user.name, buyerWhatsapp:wapp, paymentMessage:payMsg, status:"paid", createdAt:new Date().toISOString() }, ...p]);
        ...
        {step===2&&(
          <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:16, padding:22 }}>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              <textarea style={{ ...st.input, resize:"vertical", minHeight:90, lineHeight:1.6 } as any} placeholder={`Ex : J'ai envoyé ${fmt(totalFCFA)} FCFA sur ${pi.reseau||"le réseau"} ${pi.numero||""}`} value={payMsg} onChange={e=>setPayMsg(e.target.value)} />
            </div>
